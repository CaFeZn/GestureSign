...
| [#128](https://github.com/TransposonY/GestureSign/issues/128), [#120](https://github.com/TransposonY/GestureSign/issues/120) | Win11 平板/触摸屏解析对缺少标准 contact-count 或坐标范围数据的 HID 报告更宽容，会按 Win32 要求初始化 raw-device info buffer，支持无序 tip usage，改进 raw-input 诊断，不再仅因标准 `TouchScreenUsage` 设备路径里带 `ROOT` 或 `VIRTUAL_DIGITIZER` 就拒绝该设备，并把输入源 stale-timeout 的刷新改为基于完整输出帧，降低卡住的触摸屏 raw 包流持续阻塞后续触摸板输入的概率；但仍需要具体设备验证。 |
