...
- 鼠标手势在普通运行时默认关闭。如需在训练窗口之外用鼠标画手势，打开 `Options`，启用 `Mouse Gesture`，并选择一个或多个绘制按钮。启用该选项时默认使用鼠标右键。
- 在手势训练窗口里，即使全局 `Mouse Gesture` 选项仍然关闭，GestureSign 现在也接受临时的鼠标右键绘制，这样无需先改全局鼠标设置，也能直接录制鼠标手势样本。
...
- 实现路径：`PointCapture` 负责开始捕获，并在接受到有效移动时持续发出 `PointCaptured` 更新；`ContinuousGestureTrigger` 在捕获仍处于活动状态时监听这些更新，把最新点和上一次接受的点做比较，判断主方向是 `Up`、`Down`、`Left` 还是 `Right`，再根据移动距离、当前速度、经过时间，以及按当前屏幕 DPI 缩放后的 `Options` > `Continuous Gesture Distance` 计算本次应该触发几次。
- 每跨过一次完整阈值，就会立刻触发一次匹配的连续动作。移动更快时，一次更新里也可能触发多次。如果这一段捕获里已经触发过连续动作，那么同一段输入在抬手后的普通手势匹配会被取消，避免同一段操作把命令执行两遍。
- 时序可以按下面理解：
  1. 输入开始后，`PointCapture` 先判断这一段 stroke 是否允许进入捕获。
  2. 像单指触摸板边缘保护这类“捕获开闸条件”，是在 stroke 开始时判断，不是等中途再补判。
  3. 每次接受到新的有效移动，都会发出 `PointCaptured`；`ContinuousGestureTrigger` 会把最新接受点与上一批接受点做比较。
  4. 一旦移动量跨过由 `Continuous Gesture Distance`、当前速度和屏幕 DPI 共同决定的当前有效阈值，GestureSign 就会在输入仍然进行时立刻触发对应连续动作。
  5. 如果这段捕获里并没有真正触发任何“当前可执行”的连续动作，后续普通抬手手势仍可继续生效；如果已经触发过连续动作，则同一段输入的普通抬手结果会被压掉。
- 配置步骤：
  1. 创建或编辑一个动作。
...
- 这里要特别注意：触摸开始时还不知道最终会是 `Up`、`Down`、`Left` 还是 `Right`。这一刻只是决定“这一指触摸板捕获能不能开始”，真正的方向要等后续移动时才确定。
...
- 这里要注意修饰键时机：受保护的一指触摸板捕获会在触摸开始时检查 `key_is_alt_down`。手指先落下、后按 `Alt`，不会把这次普通一指移动“追补”成捕获；已经开始捕获后再松开 `Alt`，也不会单独把当前连续动作立刻取消。
...
- 当同一作用域里同一个手势同时存在“带条件动作”和“无条件兜底动作”时，只要带条件动作命中，就会优先执行它，不再和无条件兜底动作打架。这对受保护的一指边缘工作流尤其重要。
- `key_is_alt_down`、`key_space_down` 这类普通手势按键条件，现在按捕获开始时的按键快照求值，而不是按更晚的动作执行时刻求值，带条件手势会更稳定。
- 单指 touch 设备在捕获被拒绝、取消或正常完成后，现在会立即释放 source ownership，降低一次短暂失败的触摸屏/触摸板起手把下一次 touch 输入源继续卡住的概率。
- 触控笔现在可以使用简单 `Tip` 或 `Hover` 绘制，不再强制要求侧键作为必需门槛；只用 `Hover` 的模式下，后续笔尖接触屏幕也会继续保持当前笔 stroke，而不会中途丢掉。
...
- 单指触摸屏动作现在也可以使用与受保护单指触摸板动作相同的“带条件显式开闸”模型。在应用专属作用域里，只要你明确配置了单指触摸屏动作，它就可以不依赖额外条件越过默认的每应用手指数限制进入单指捕获；在正常 global fallback 允许时，全局的一指触摸屏动作现在也可以如此。如果你只希望其中一部分起手能触发，再额外添加触发条件即可。
...
- 现在如果某个标准 HID `PenUsage` 设备的 raw device path 里带有 `ROOT` 或 `VIRTUAL_DIGITIZER`，也不会再仅因为这个路径形式就被提前挡在手势捕获之外；这和前面对标准 HID 触摸屏、触摸板的 Win11 宽容处理保持一致。`使用笔时忽略触摸输入` 仍然保持更窄的信任边界，不会自动把所有这类 pen 路径都当成适合做 touch suppression。
...
- 连续手势会在手指仍在移动时运行，但必须先超过由 `Options` > `Continuous Gesture Distance`、当前速度和捕获手势所在屏幕 DPI 共同决定的有效阈值。如果短滑动在超过这个有效阈值前就松开，匹配的普通手势仍可能先运行。
...
- 如果某个 Win11 平板的标准 `TouchScreenUsage` 触摸屏设备路径里带有 `ROOT` 或 `VIRTUAL_DIGITIZER`，现在也不会再仅因为这两个片段就被提前过滤；若仍然无法使用，请结合校验日志确认 Windows 实际暴露的是 `TouchScreenUsage`，而不只是鼠标、触控笔或厂商私有输入。
- daemon 日志现在也会记录一次性的 raw digitizer 设备校验信息，包括 vendor ID、product ID、raw usage、effective usage、device path，以及该设备最终是被接受还是被忽略。对于 Win11 下的第三方触摸板驱动，例如 reWASD 或 Apple Magic Trackpad 驱动，这是确认 Windows 实际向 GestureSign 暴露了什么输入类型的最快方式。
- 某些 Win11 第三方驱动会把本质上像触摸板的设备暴露成 `TouchScreenUsage` 或其他 digitizer 顶层集合。当前构建会尽量把明显像 trackpad 的别名设备路由到触摸板路径，并在校验日志里用 `rawUsage=...` 和 `effectiveUsage=TouchPad` 明确标出来。
...
- 现在的构建在一个 touch 设备被另一个 touch 设备挡住，或后来者接管旧 source 时，也会输出一次性的 `Touch source arbitration:` 日志。对于 Win11 下“触摸屏碰一下后触摸板也像被拖住”的问题，这类日志能直接看出当时系统认为哪台设备还活跃、后来者是否因为旧 source 超时而成功接管。
...
- 当前构建也不再默认把第一个 digitizer link collection 下的每个 child 都当成真实触点。触摸板和触摸屏解析现在只会使用那些真正暴露了 X/Y 坐标能力的 link collection，这对 Win11 下把非 contact 子集合混进同一顶层 collection 的第三方 HID 布局更宽容。
...
- `Block Touch Input` 仅在 UIAccess 构建中可用，并按匹配应用配置。它会在当前触摸帧达到设定触点阈值后开始阻断，所以阈值以下的很早期触摸帧仍可能传给目标应用。它只重定向 touch pointer 帧，不是对精确触摸板、触控笔、鼠标、键盘或应用原生输入的完整系统事件阻断器。
...
- 对单指触摸板连续工作流来说，安全模式就是“足够窄的 `start_*` 起始区域 + 可选修饰键保护 + 移动过程中重复命令”，而不是先普通一指移动，再靠后续 `end_*` 进入某个区域后才开始拦截。
...
| [#109](https://github.com/TransposonY/GestureSign/issues/109), [#131](https://github.com/TransposonY/GestureSign/issues/131) | 对切换窗口/桌面动作的 shell/taskbar 激活和修饰键释放做了加固。内置的 Alt+Tab / Shift+Alt+Tab / Win+Ctrl+方向键动作，以及等价的 `Hot Key` 快捷键，现在在成功路径上也会强制复位相关修饰键，而不只是依赖普通的 key-up 注入；并且在系统已经完成切窗/切桌面后，这一步修饰键清理现在会保留系统切换后真正的前台窗口，不再把焦点留在临时 shield window 上，也不会把切换前的目标窗口强行抢回前台；通用命令执行路径在发送按键前也会再次把 shell/taskbar 漂移纠正回真实的捕获目标窗口。如果手势本身就是从任务栏上起手，而且那一刻按坐标命中的窗口和当前前台都已经落到 shell UI，命令执行阶段的窗口解析现在也会回退到最近一次记住的非 shell 前台窗口，而不是继续卡在 taskbar 上。 |
...
| [#57](https://github.com/TransposonY/GestureSign/issues/57), [#9](https://github.com/TransposonY/GestureSign/issues/9) | 对 touch blocking 做了加固，指针捕获丢失/取消帧会释放内部注入触点 ID，并文档化 UIAccess、按应用配置、初始帧等限制；当前触摸帧一旦达到配置阈值，就不会再仅因为手势捕获仍处于临时 `CapturingInvalid` 阶段而把这批触摸重新注回底层应用。 |
...
| [#79](https://github.com/TransposonY/GestureSign/issues/79), [#81](https://github.com/TransposonY/GestureSign/issues/81) | Windows Store/UWP 启动和匹配路径已存在，包括 `Launch Windows Store App` 以及把 `ApplicationFrameWindow` 展开到 `Windows.UI.Core.CoreWindow`。当这种精确的内层类名不存在时，窗口解析现在也会保守地回退到包装窗口下“唯一可见且非 shell 的 descendant”，降低 Microsoft 应用长期停留在 `ApplicationFrameWindow` 包装层元数据上的概率；命令执行时 `PointInfo` 的窗口解析也会沿用这套真实窗口展开逻辑，不再在后续插件代码里回退到包装窗口。对运行中的 Windows 应用新建规则时，应用对话框现在不仅会对直接命中的 `Windows.UI.Core.CoreWindow` 目标优先选择 `Window Class` 匹配，对那些先从 `ApplicationFrameWindow` 包装层解包出来的 live 目标也会如此；同时 `Launch Windows Store App` 在直接 `ActivateApplication(AUMID)` 激活失败时，也会回退到 `explorer.exe shell:AppsFolder\<AUMID>` 启动路径；但 Microsoft 应用剩余失败仍需要按应用和 package 状态验证。 |
| [#85](https://github.com/TransposonY/GestureSign/issues/85) | 触摸板延迟、重复和轨迹锯齿仍有一部分取决于驱动/设备，但触摸板/raw-input 处理已继续改进：完整逻辑槽位解析、按设备句柄区分同类输入源、让实时多指更新按稳定触点顺序参与计算，以及在“已经移动中的多指触摸板 stroke 里驱动一次性换掉整组 contact id”时尽量续接当前手势，能降低部分硬件上“来回抖动”或连续手势方向重复翻转的问题；最终关闭仍需要受影响的 Synaptics 或厂商驱动硬件验证。 |
| [#103](https://github.com/TransposonY/GestureSign/issues/103) | 显示器变化处理现在会释放活跃 raw input、清理触摸屏幕缓存、在 UI 消息上下文重新注册 raw input，并且在消息窗口句柄变化期间如果重新注册失败也只会记录并清理状态，不再把异常继续向外冒；设备状态延迟刷新现在也会明确回投到保存下来的同步上下文执行，不再依赖电源恢复或会话事件触发时碰巧所在的上下文；如果一次显示拓扑刷新尚未跑完时又来了新的 `DisplaySettingsChanged`，现在也会在当前轮结束后补跑最后一次待处理刷新；如果 PointCapture 自己的回投失败，现在也会回退到当前线程直接执行，不再把这类更新静默丢掉；同时会重置手势轨迹表面，并在系统临时拿不到显示器边界时跳过输入帧；外接显示器热拔仍需要在对应笔记本/外接显示拓扑上验证。 |
| [#128](https://github.com/TransposonY/GestureSign/issues/128), [#120](https://github.com/TransposonY/GestureSign/issues/120) | Win11 平板/触摸屏解析对缺少标准 contact-count 或坐标范围数据的 HID 报告更宽容，会按 Win32 要求初始化 raw-device info buffer，支持无序 tip usage，改进 raw-input 诊断，不再仅因标准 `TouchScreenUsage` 设备路径里带 `ROOT` 或 `VIRTUAL_DIGITIZER` 就拒绝该设备；对于标准 `TouchScreenUsage` / `TouchPadUsage` 设备，即使 `RIDI_DEVICENAME` 读不到，也不再直接把整台设备判死；当触摸屏在移动中的手势里重编号 contact ID、但坐标仍能匹配当前轨迹时，也会尽量续接当前捕获；同时，一旦前一台 touch 设备已经超过 stale timeout，即使旧 capture 状态还没及时清干净，后来的 touch 输入源也可以接管；如果那台旧 touch 设备甚至一直没走出 `CapturingInvalid` 阶段，这个接管超时还会更短，进一步降低“触屏碰一下后触摸板也一直被拖住直到重启”的概率；但仍需要具体设备验证。 |
| [#123](https://github.com/TransposonY/GestureSign/issues/123) | 快速点击时，如果新的活跃触点帧替换了过期触点集合，现在不会再直接丢掉该帧；同时会拆分“上一轮释放 + 下一轮按下”合并在同一 raw 帧里的情况，让上一次 tap 先结束、下一次 tap 再开始；如果上一段触摸捕获还停留在 invalid 阶段，只要同一轮输入里的触点集合发生变化，现在也会立刻按新触点集合重建该段捕获，而不再要求一定是某一种特定的“旧释放 + 新按下”组合；当新的触点组要在同一条更新里接管当前手势时，现在也会先把上一段 touch capture 正常收尾，再启动新的一段；在“释放 + 下一次按下”共享同一条 raw 更新的情况下，touch-up 交接现在也会尽量只带上真正释放的已跟踪触点，而不是把仍然活跃的新触点混进去；对同一段极短交接抖动里反复触发的 touch capture 重启，现在仍会抑制，但不会再额外拖慢真实的手指数增加；组合手势/tap 后续等待也只会在该手势对当前设备和条件确实可执行时才继续成立；但高频触摸屏场景仍需要真实硬件验证。 |
...
| [#127](https://github.com/TransposonY/GestureSign/issues/127), [#119](https://github.com/TransposonY/GestureSign/issues/119), [#112](https://github.com/TransposonY/GestureSign/issues/112), [#55](https://github.com/TransposonY/GestureSign/issues/55) | 触控笔设置和文档更清晰，简单笔手势也不再强制要求侧键，而且标准 `PenUsage` 设备现在也不会再仅因 raw HID 路径里带 `ROOT` 或 `VIRTUAL_DIGITIZER` 就被提前拒绝；但 Wacom/无源笔支持仍取决于驱动是否暴露兼容的 HID pen 或 touchpad 输入。 |
| [#135](https://github.com/TransposonY/GestureSign/issues/135), [#116](https://github.com/TransposonY/GestureSign/issues/116), [#114](https://github.com/TransposonY/GestureSign/issues/114), [#59](https://github.com/TransposonY/GestureSign/issues/59) | 精确触摸板处理已有改进，包括更安全的一指捕获开闸：会先检查应用专属的带条件触摸板动作，再在允许时回退到全局动作；同时，标准 `TouchPadUsage` 设备不再仅因 raw HID 设备路径里带 `ROOT` 或 `VIRTUAL_DIGITIZER` 就被过滤掉，一些明显像 trackpad 的 digitizer 报告即使原始顶层 usage 更像 `TouchScreenUsage` 或其他非标准 digitizer collection，也会尽量被路由到触摸板路径；触摸解析现在也只会遍历那些真正暴露坐标能力的 link collection，而不再把每个 child 都盲当成 contact；同类触摸板源也会按设备句柄区分，降低多触摸板之间互相干扰的概率；raw input 设备清单和 digitizer 校验现在还会记录 vendor/product 以及 raw/effective usage 细节，加快 Win11 第三方驱动诊断，但第三方/厂商驱动支持仍必须按设备验证。 |
| [#130](https://github.com/TransposonY/GestureSign/issues/130), [#87](https://github.com/TransposonY/GestureSign/issues/87), [#45](https://github.com/TransposonY/GestureSign/issues/45) | 连续捕获期间触发的鼠标 hold-down 动作会在捕获结束/取消时自动释放，降低鼠标键卡住风险；同时 hold-down 是否进入自动释放追踪，现在也会按“连续动作真正触发当时”的捕获状态判断，而不再依赖稍后异步命令执行时的当前状态；完整窗口拖拽/文件拖拽工作流仍需要硬件和 UI 验证。 |
...
- 每次推送到 `master` 也会更新 `continuous` 预发布版，提供最新安装版和便携版构建用于手动测试。
...
- 也可以在 GitHub Actions 页面手动运行 `Release` workflow。填写 `tag_name`；测试预发布版可用 `continuous`，正式 release 使用 semver 风格 tag。也可以额外填写 `build_ref` 来构建指定 branch、commit 或 tag。如果不填写 `build_ref`，semver 风格 tag 默认构建该 tag；`continuous` 则默认构建你发起 workflow 时所在 ref 的那次 SHA。
