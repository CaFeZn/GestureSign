...
- 时序可以按下面理解：
  ...
  4. 一旦移动量跨过当前阈值，GestureSign 就会在输入仍然进行时立刻触发对应连续动作。
