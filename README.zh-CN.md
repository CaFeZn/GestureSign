...
| [#128](https://github.com/TransposonY/GestureSign/issues/128), [#120](https://github.com/TransposonY/GestureSign/issues/120) | Win11 平板/触摸屏解析对缺少标准 contact-count 或坐标范围数据的 HID 报告更宽容，会按 Win32 要求初始化 raw-device info buffer，支持无序 tip usage，改进 raw-input 诊断，不再仅因标准 `TouchScreenUsage` 设备路径里带 `ROOT` 或 `VIRTUAL_DIGITIZER` 就拒绝该设备；当触摸屏在移动中的手势里重编号 contact ID、但坐标仍能匹配当前轨迹时，也会尽量续接当前捕获；同时，一旦前一台 touch 设备已经超过 stale timeout，即使旧 capture 状态还没及时清干净，后来的 touch 输入源也可以接管；如果那台旧 touch 设备甚至一直没走出 `CapturingInvalid` 阶段，这个接管超时还会更短，进一步降低“触屏碰一下后触摸板也一直被拖住直到重启”的概率；但仍需要具体设备验证。 |
