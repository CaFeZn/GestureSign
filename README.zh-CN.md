...
- 本 fork 便携版：当本 fork 已有 GitHub Release 时，从 [releases 页面](https://github.com/CaFeZn/GestureSign/releases) 下载 `GestureSign-<tag>-win-anycpu.zip`，解压到目标目录后运行 `GestureSign.ControlPanel.exe`。
...
- 如果浏览器动作无法匹配 Chromium Edge，请用 executable filename 或 process matching 添加/编辑 `msedge.exe` 的应用规则。内置浏览器组会匹配常见浏览器进程，包括 `msedge`、`chrome`、`firefox`、`iexplore` 和旧版 `MicrosoftEdge`。
...
| [#83](https://github.com/TransposonY/GestureSign/issues/83), [#89](https://github.com/TransposonY/GestureSign/issues/89), [#69](https://github.com/TransposonY/GestureSign/issues/69) | 可用手指坐标、窗口状态和按住的修饰键触发条件近似条件手势、区域手势和边缘手势工作流，但没有专门的边缘手势识别器或悬浮球。 |
