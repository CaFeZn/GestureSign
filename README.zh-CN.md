...
| [#109](https://github.com/TransposonY/GestureSign/issues/109), [#131](https://github.com/TransposonY/GestureSign/issues/131) | 对切换窗口/桌面动作的 shell/taskbar 激活和修饰键释放做了加固。内置的 Alt+Tab / Shift+Alt+Tab / Win+Ctrl+方向键动作，以及等价的 `Hot Key` 快捷键，现在在成功路径上也会强制复位相关修饰键，而不只是依赖普通的 key-up 注入；并且在系统已经完成切窗/切桌面后，这一步修饰键清理现在会保留系统切换后真正的前台窗口，不再把焦点留在临时 shield window 上，也不会把切换前的目标窗口强行抢回前台；通用命令执行路径在发送按键前也会再次把 shell/taskbar 漂移纠正回真实的捕获目标窗口。 |
