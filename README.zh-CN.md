...
| [#103](https://github.com/TransposonY/GestureSign/issues/103) | 显示器变化处理现在会释放活跃 raw input、清理触摸屏幕缓存、在 UI 消息上下文重新注册 raw input，并且在消息窗口句柄变化期间如果重新注册失败也只会记录并清理状态，不再把异常继续向外冒；同时会重置手势轨迹表面，并在系统临时拿不到显示器边界时跳过输入帧；外接显示器热拔仍需要在对应笔记本/外接显示拓扑上验证。 |
