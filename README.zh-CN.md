...
| [#79](https://github.com/TransposonY/GestureSign/issues/79), [#81](https://github.com/TransposonY/GestureSign/issues/81) | Windows Store/UWP 启动和匹配路径已存在，包括 `Launch Windows Store App` 以及把 `ApplicationFrameWindow` 展开到 `Windows.UI.Core.CoreWindow`；命令执行时 `PointInfo` 的窗口解析现在也会沿用这套真实窗口展开逻辑，不再在后续插件代码里回退到包装窗口。对运行中的 Windows 应用新建规则时，应用对话框现在也会对 `Windows.UI.Core.CoreWindow` 目标优先选择 `Window Class` 匹配，而不是再默认回到可执行文件名；同时 `Launch Windows Store App` 在直接 `ActivateApplication(AUMID)` 激活失败时，也会回退到 `explorer.exe shell:AppsFolder\<AUMID>` 启动路径；但 Microsoft 应用剩余失败仍需要按应用和 package 状态验证。 |
