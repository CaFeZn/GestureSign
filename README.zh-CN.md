...
| [#134](https://github.com/TransposonY/GestureSign/issues/134), [#122](https://github.com/TransposonY/GestureSign/issues/122) | 连续手势距离可配置、已文档化，修复了刚到阈值时首个触发被丢弃的问题，让距离阈值按手势所在屏幕 DPI 缩放；当横纵位移恰好打平时，也不再默认按垂直方向触发连续手势；并且只有当前确实可执行的连续动作真正触发后，才会压掉同一段输入的普通手势。 |
...
| [#103](https://github.com/TransposonY/GestureSign/issues/103) | 显示器变化处理现在会释放活跃 raw input、清理触摸屏幕缓存、在 UI 消息上下文重新注册 raw input，并且在消息窗口句柄变化期间如果重新注册失败也只会记录并清理状态，不再把异常继续向外冒；设备状态延迟刷新现在也会明确回投到保存下来的同步上下文执行，不再依赖电源恢复或会话事件触发时碰巧所在的上下文；同时会重置手势轨迹表面，并在系统临时拿不到显示器边界时跳过输入帧；外接显示器热拔仍需要在对应笔记本/外接显示拓扑上验证。 |
