...
- 本 fork 安装版：从 [releases 页面](https://github.com/CaFeZn/GestureSign/releases) 下载 `GestureSign-<tag>-setup-win-anycpu.exe` 并运行。
...
- 鼠标绘制按钮是触发按钮，不是输出动作。`Mouse Actions` 可以在手势匹配后发送滚轮输入，但单独滚动滚轮不是鼠标手势绘制触发器。
...
- release workflow 已在上游支持时使用兼容 Node 24 的 checkout 和 release action；`actions/upload-artifact` 仍需等待上游发布兼容 Node 24 的 major 版本后，才能完全消除 Actions 警告。
