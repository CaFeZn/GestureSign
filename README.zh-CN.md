...
| [#79](https://github.com/TransposonY/GestureSign/issues/79), [#81](https://github.com/TransposonY/GestureSign/issues/81) | Windows Store/UWP 启动和匹配路径已存在，包括 `Launch Windows Store App` 以及把 `ApplicationFrameWindow` 展开到 `Windows.UI.Core.CoreWindow`；命令执行时 `PointInfo` 的窗口解析现在也会沿用这套真实窗口展开逻辑，不再在后续插件代码里回退到包装窗口；但 Microsoft 应用失败仍需要按应用和 package 状态验证。 |
