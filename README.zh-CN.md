...
| [#113](https://github.com/TransposonY/GestureSign/issues/113) | 应用专属的同手势动作即使命令被禁用，也会阻止回退到全局动作，因此可以只在某个应用中排除一个全局手势，而不影响该应用中的其他全局手势。启用 `匹配当前激活的窗口` 的应用规则现在也会从捕获开始阶段一致生效，独立带条件滚轮触发也会走同一套激活窗口匹配逻辑，让按应用限制和受保护触摸板捕获与最终命中的应用保持一致。 |
