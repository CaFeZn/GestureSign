...
| [#128](https://github.com/TransposonY/GestureSign/issues/128), [#120](https://github.com/TransposonY/GestureSign/issues/120) | Win11 平板/触摸屏解析对缺少标准 contact-count 或坐标范围数据的 HID 报告更宽容，会按 Win32 要求初始化 raw-device info buffer，支持无序 tip usage，改进 raw-input 诊断，不再仅因标准 `TouchScreenUsage` 设备路径里带 `ROOT` 或 `VIRTUAL_DIGITIZER` 就拒绝该设备；当触摸屏在移动中的多指手势里重编号 contact ID、但坐标仍能匹配当前轨迹时，也会尽量续接当前捕获；同时输入源 stale-timeout 的刷新改为基于完整输出帧，降低卡住的触摸屏 raw 包流持续阻塞后续触摸板输入的概率；但仍需要具体设备验证。 |
| [#123](https://github.com/TransposonY/GestureSign/issues/123) | 快速点击时，如果新的活跃触点帧替换了过期触点集合，现在不会再直接丢掉该帧；同时会拆分“上一轮释放 + 下一轮按下”合并在同一 raw 帧里的情况，让上一次 tap 先结束、下一次 tap 再开始；如果同一条 raw 更新里既有已跟踪触点的释放、又有新的触点加入，而上一段捕获还停留在 invalid 阶段，现在也会先正确结束上一段，再开始下一段；当新的触点组要在同一条更新里接管当前手势时，现在也会先把上一段 touch capture 正常收尾，再启动新的一段；组合手势/tap 后续等待也只会在该手势对当前设备和条件确实可执行时才继续成立；但高频触摸屏场景仍需要真实硬件验证。 |
