...
- Pen gestures require `Options` > `Pen Gesture` and at least one draw mode (`Tip` or `Hover`). The barrel/right-click button or inverted pen can now be used as an optional extra activation gate instead of a mandatory requirement. If a stylus driver exposes its button as a normal mouse right-click instead of HID pen input, try `Mouse Gesture` with the right mouse button.
