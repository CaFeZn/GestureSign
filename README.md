...
- Timing summary:
  ...
  4. After movement crosses the current effective threshold derived from `Continuous Gesture Distance`, current speed, and screen DPI, GestureSign fires the matching continuous action immediately, while the input is still active.
...
- Touch start still does not know the final direction yet. At that moment GestureSign only decides whether one-finger touchpad capture may begin; the later movement decides whether the continuous action is `Up`, `Down`, `Left`, or `Right`.
...
- One-finger touchscreen actions can now use the same conditioned opt-in model as guarded one-finger touchpad actions. In app-specific scopes, an explicit one-finger touchscreen action can opt into one-finger capture without requiring an extra condition just to get past the default per-app finger limit. When normal global fallback is allowed, a global one-finger touchscreen action can now do the same. Add a trigger condition when you want that one-finger touch action to be gated to only a subset of starts instead of always capturing in that scope.
- Pen gestures require `Options` > `Pen Gesture` and at least one draw mode (`Tip` or `Hover`). The barrel/right-click button or inverted pen can now be used as an optional extra activation gate instead of a mandatory requirement. Turning the option on now defaults to simple `Tip` drawing first. If a stylus driver exposes its button as a normal mouse right-click instead of HID pen input, try `Mouse Gesture` with the right mouse button.
- For passive pens or pens without a usable barrel button, turn on `Pen Gesture`, leave `Tip` enabled, and leave the optional activation-gate checkboxes off. If the pen still does not start drawing, the remaining blocker is usually whether the driver exposes compatible HID pen input to Windows.
- Standard HID pen devices are now accepted for gesture capture even when their raw device path contains `ROOT` or `VIRTUAL_DIGITIZER`, matching the same Win11-oriented tolerance already used for standard HID touchscreens and touchpads. `Ignore Touch Input When Using Pen` still keeps a narrower trust boundary and does not automatically treat every such pen path as safe for touch suppression.
- In `Hover`-only pen mode, a later tip contact now keeps the active pen gesture alive instead of dropping the stroke state mid-gesture.
- `Options` > `Ignore Touch Input When Using Pen` now suppresses touch only while the pen is reporting an active HID pen state, instead of reacting to every pen raw-input packet. This reduces false touch suppression from passive hover noise or idle pen reports on some Win11 devices.
- Continuous gestures run while fingers are still moving, but only after movement crosses an effective threshold derived from `Options` > `Continuous Gesture Distance`, current speed, and the DPI of the screen where the gesture is captured. If a short swipe releases before that effective threshold, the matching normal gesture can still run first.
- `Options` > `Minimum Point Distance` has an effective minimum of `3 px`, and `Options` > `Continuous Gesture Distance` has an effective minimum of `5 px`. Stored values below those limits are normalized to the same minima the control panel already enforces.
...
- `Drawing Start Timeout` uses an effective range of `0..2.0 s`, `Whole Gesture Timeout` uses `0..10.0 s`, and `Composite Gesture Timeout` uses `0..2.0 s`. Saved values outside those control-panel ranges are normalized back into the same bounds at runtime.
...
- Standard HID touchscreen devices are also no longer filtered only because their raw device path contains `ROOT` or `VIRTUAL_DIGITIZER`. If a Win11 tablet still does not work, check the validation logs to confirm Windows really exposes `TouchScreenUsage`, not only mouse, pen, or vendor-specific reports.
- The daemon log now records one-time raw digitizer device-validation lines with vendor ID, product ID, raw usage, effective usage, device path, and whether the device was accepted or ignored. For Win11 third-party touchpad reports such as reWASD or Apple Magic Trackpad drivers, this is the fastest way to confirm what Windows is actually exposing to GestureSign.
- Some third-party Win11 drivers expose a trackpad through `TouchScreenUsage` or another digitizer top-level collection even though it behaves like a touchpad. Current builds can route obvious trackpad-like aliases through the touchpad path, and the validation log shows that remap as `rawUsage=...` and `effectiveUsage=TouchPad`.
- The daemon log also records one-time raw input device inventory lines, including devices that Windows exposes only as mouse, keyboard, or non-digitizer HID. For third-party Win11 touchpad reports, collect both the `Enumerated raw input device:` lines and the `Raw digitizer device validation:` lines from `GestureSign.log`.
- Current builds also log one-time `Touch source arbitration:` lines when one touch device is blocked by, or takes over from, another. For Win11 tablet reports where touchscreen input seems to leave the touchpad blocked, those lines show which source was considered active, which source tried to take over, the capture state, and whether takeover happened because the old source timed out.
- Current builds emit those raw-input inventory/validation lines automatically during daemon startup and later device-state refreshes, so you usually only need to reproduce the input problem once before checking the log.
- Some precision-touchpad drivers also report only the currently active touch count while leaving live contacts in later logical slots. GestureSign now parses the full logical touchpad report width in that case, so later-slot touchpad contacts are less likely to be skipped.
- Current builds also stop assuming that every child of the first digitizer link collection is a real touch contact. Touchpad and touchscreen parsing now use only link collections that actually expose X/Y coordinate caps, which is more tolerant of Win11 third-party HID layouts that mix non-contact children into the same top-level collection.
...
- Touchpad timeout/cancel recovery now keeps the temporary ignore-until-up state on the same physical/raw device handle, instead of muting every touchpad source of that class together.
- When a touch-device capture times out or is canceled, GestureSign now also clears the current touch-source ownership for that same device immediately, instead of waiting for another raw frame from that device before other same-class devices can take over.
- The same timeout/cancel path now releases the underlying raw-input touch-source lock immediately as well, so another touch device does not need to wait for the old source's stale timeout to expire before taking over.
- The same immediate release now also runs when a touch-device capture is rejected right at capture start by current app rules, whitelist mode, or finger-count limits, so a briefly rejected touchscreen start is less likely to keep the next touchpad source waiting.
- The same immediate release now also runs when the finger-up recognition phase is canceled after capture, for example because a continuous action already fired and suppressed the normal gesture result.
- The same immediate release now also runs after a normal completed touch-device gesture, so the next touchscreen or touchpad source does not need to wait for stale source ownership after an ordinary finger-up completion either.
- This immediate release path now covers touchscreen captures too, so a canceled or timed-out touchscreen stroke is less likely to keep the next touch-device source waiting.
...
- `Block Touch Input` is available only in UIAccess builds and is configured per matched application. It starts blocking once the current touch frame reaches the configured contact threshold, so very early sub-threshold touch frames may still reach the target app. It redirects touch pointer frames only; it is not a complete system-event blocker for precision-touchpad, pen, mouse, keyboard, or app-specific native input.
- `Block Touch Input` values below `2` are treated as `Off` consistently now, so the dialog, saved user settings, and bundled defaults all use the same threshold semantics.
- Per-application `Number Of Fingers` is normalized to `1..10`, and per-application `Block Touch Input` is normalized to `0..10` with values below `2` treated as `Off`, matching the bounds enforced by the application dialog.
- If Windows rejects a touch-blocking unregister attempt, GestureSign now still clears the in-memory pointer/frame bookkeeping for that interception round, reducing the chance that stale touch-blocking state leaks into later input handling.
- Re-enabling or disabling `Block Touch Input` now clears the previous pointer-ID frame state before the next interception round, so stale injected-touch bookkeeping is less likely to leak across app switches or threshold changes.
...
- When the same gesture has both conditioned actions and a plain fallback action in the same scope, GestureSign now prefers the conditioned actions that actually matched instead of running the unconditional fallback as well.
...
- Composite-gesture waiting and tap follow-up matching now consider whether the matched gesture action is actually executable for the current device, command state, and trigger condition, instead of only checking whether an action with that gesture name exists somewhere.
...
- Touchscreen screen resolution is now more conservative when the current frame cannot be matched reliably. If a cached touchscreen-to-screen mapping conflicts with either a unique current foreground-window touch match or a unique physical-size match, GestureSign refreshes that cache to the newly matched screen. If cached mapping is unavailable and both heuristics fail, GestureSign skips that unreliable touchscreen frame instead of blindly falling back to the foreground or cursor screen.
- For cursor-driven pen and touchpad fallback paths, screen selection now prefers the cursor's current monitor before the foreground-window monitor. That keeps those inputs aligned with the same monitor used by cursor-based app matching and touchpad start conditions.
...
- If a Win11 third-party touchpad driver is not detected, restart the tray daemon, reproduce the problem once, then open `%LOCALAPPDATA%\GestureSign\GestureSign.log` and search for `Enumerated raw input device:` plus `Raw digitizer device validation:`. Those lines show whether Windows exposed the device as `TouchPad`, some other HID usage, or only as mouse/keyboard input.
...
- Active touch captures are also protected against stale-timeout source theft now, so another touch device is less likely to steal ownership mid-gesture just because the current contact paused without fully ending.
...
| [#138](https://github.com/TransposonY/GestureSign/issues/138), [#95](https://github.com/TransposonY/GestureSign/issues/95), [#62](https://github.com/TransposonY/GestureSign/issues/62) | Single-finger touchscreen gestures and one-finger touchscreen tap training are supported. App-specific one-finger touchscreen actions no longer need an extra condition just to get past the default per-app finger-count limit, and global one-finger touchscreen actions can now also do so when normal global fallback is allowed; conditioned one-finger touchscreen actions still work as an opt-in gate when you want only a subset of touch starts to capture. |
...
| [#136](https://github.com/TransposonY/GestureSign/issues/136), [#90](https://github.com/TransposonY/GestureSign/issues/90) | Gesture training help now explains which input sources can draw. When general mouse gestures are off, training mode still accepts a temporary right-button mouse stroke so new users can record a gesture immediately, and the training UI can also enable the normal default right-button mouse-gesture configuration permanently when desired. |
| [#134](https://github.com/TransposonY/GestureSign/issues/134), [#122](https://github.com/TransposonY/GestureSign/issues/122) | Continuous gesture distance is configurable, documented, threshold-edge firing no longer drops the first trigger, distance thresholds scale with the gesture screen DPI, exact diagonal ties no longer default to a vertical continuous-fire decision, and a normal gesture is now suppressed only after a currently executable continuous action actually fires, including the final finger-up path after capture-end events have already run. |
...
| [#109](https://github.com/TransposonY/GestureSign/issues/109), [#131](https://github.com/TransposonY/GestureSign/issues/131) | Shell/taskbar activation and modifier-key cleanup around switch-window and switch-desktop actions have been hardened. Built-in Alt+Tab / Shift+Alt+Tab / Win+Ctrl+Arrow actions, and equivalent `Hot Key` shortcuts, now force-reset the relevant modifiers on the success path as well instead of only relying on best-effort key-up injection, that cleanup now preserves the real foreground window after the system switch has already completed instead of leaving focus on a temporary shield window or restoring the pre-switch target, and generic command execution now re-resolves shell/taskbar foreground drift back to the real captured target before sending keystrokes. When a gesture starts over the taskbar and both the point-hit window and current foreground are shell UI, command-time window resolution now also falls back to the last known non-shell foreground window instead of staying stuck on the taskbar. |
...
| [#75](https://github.com/TransposonY/GestureSign/issues/75), [#32](https://github.com/TransposonY/GestureSign/issues/32) | Trigger conditions support touch-coordinate variables, multi-condition expressions, and percent coordinates aligned with Win32 virtual-screen pixel bounds. Multi-contact condition evaluation now also reads points and contact identifiers from one explicit ordered capture snapshot, so variables such as `finger_2_start_X` are less likely to drift onto the wrong tracked contact during later execution paths. |
| [#66](https://github.com/TransposonY/GestureSign/issues/66), [#24](https://github.com/TransposonY/GestureSign/issues/24), [#52](https://github.com/TransposonY/GestureSign/issues/52) | Trigger conditions can test arbitrary held virtual keys such as `key_space_down`, `key_a_down`, and `key_page_up_down`, the condition editor can capture a key name for insertion, and normal gesture recognition now evaluates those key conditions from the capture-start key snapshot instead of the later action-execution moment. |
...
| [#57](https://github.com/TransposonY/GestureSign/issues/57), [#9](https://github.com/TransposonY/GestureSign/issues/9) | Touch-blocking behavior has been hardened, pointer capture-loss/canceled frames release internal injected-touch IDs, UIAccess/per-app/initial-frame limits are documented, and once a touch frame reaches the configured block threshold it is no longer reinjected just because the gesture capture is still in the temporary `CapturingInvalid` stage. |
| [#51](https://github.com/TransposonY/GestureSign/issues/51) | Drawing-start timeout is honored for precision touchpad gestures. |
| [#83](https://github.com/TransposonY/GestureSign/issues/83), [#89](https://github.com/TransposonY/GestureSign/issues/89), [#69](https://github.com/TransposonY/GestureSign/issues/69) | Conditioned one-finger precision-touchpad capture supports guarded edge/zone workflows, including right-edge continuous scrolling; with a narrow enough start condition, it avoids intercepting ordinary one-finger touchpad movement, and matched conditioned actions now take precedence over same-gesture unconditional fallbacks in the same scope. |
...
| [#79](https://github.com/TransposonY/GestureSign/issues/79), [#81](https://github.com/TransposonY/GestureSign/issues/81) | Windows Store/UWP launch and matching paths exist, including `Launch Windows Store App` and `ApplicationFrameWindow` unwrapping to `Windows.UI.Core.CoreWindow`. When that exact inner class is absent, window resolution now also falls back conservatively to a unique visible non-shell descendant of the wrapper, reducing cases where Microsoft apps stay stuck on `ApplicationFrameWindow` metadata alone. Command-time `PointInfo` window resolution reuses that same unwrapped real-window path instead of falling back to the wrapper window in later plugin code. When creating a new application rule from a live Windows app window, the dialog now prefers `Window Class` matching not only for direct `Windows.UI.Core.CoreWindow` targets, but also for live targets that were recovered from an `ApplicationFrameWindow` wrapper. `Launch Windows Store App` now falls back to `explorer.exe shell:AppsFolder\<AUMID>` if direct `ActivateApplication(AUMID)` activation fails; remaining Microsoft app failures still need per-app/package-state validation. |
| [#85](https://github.com/TransposonY/GestureSign/issues/85) | Touchpad delay, duplicate, and jagged-path reports are still partly driver/device-specific, but touchpad/raw-input handling has improved: full logical-slot parsing, per-device-handle tracking, stable per-contact ordering for live multi-finger updates, and touchpad-specific continuation when a driver rolls all active contact IDs at once during an already-moving multi-finger stroke all reduce false back-and-forth motion or repeated continuous-trigger direction flips on some hardware. Affected Synaptics or vendor-driver hardware is still required for full closure. |
| [#103](https://github.com/TransposonY/GestureSign/issues/103) | Display-change handling now releases active raw input, clears touchscreen screen caches, re-registers raw input on the UI message context, also traps registration failures during message-window handle changes instead of letting them bubble out, routes delayed device-state refreshes back through the saved synchronization context instead of depending on the resume/session event's current context, replays a pending display-topology refresh if another `DisplaySettingsChanged` arrives while one refresh is already running, falls back to direct execution if a queued PointCapture context-post itself fails, resets the gesture-trail surface, and skips input frames when monitor bounds are temporarily unavailable; external-monitor hot-unplug still needs validation on the reported laptop/external-display topology. |
| [#128](https://github.com/TransposonY/GestureSign/issues/128), [#120](https://github.com/TransposonY/GestureSign/issues/120) | Win11 tablet/touchscreen parsing is more tolerant of HID reports without standard contact-count or coordinate-range data, initializes raw-device info buffers per Win32 requirements, handles unordered tip usages, improves raw-input diagnostics, no longer rejects standard `TouchScreenUsage` devices only because their raw HID path contains `ROOT` or `VIRTUAL_DIGITIZER`, now also keeps standard `TouchScreenUsage` / `TouchPadUsage` devices alive even when `RIDI_DEVICENAME` cannot be read, continues a moving touch capture when touchscreen contact IDs are renumbered but points still match the current stroke, and now lets a later touch source take over once the previous touch source has exceeded the stale timeout even if the old capture state had not been cleaned up yet; if that old touch source never progressed past `CapturingInvalid`, the takeover timeout is also shorter so later touchpad input does not stay blocked as long after a broken touchscreen start. |
| [#123](https://github.com/TransposonY/GestureSign/issues/123) | Rapid tap handling no longer drops a new active-contact frame when it replaces a stale contact set, splits combined release/next-press raw frames so the previous tap can finish before the next tap starts, rebuilds a still-invalid touch capture as soon as its contact set changes instead of waiting for the old release/new-press mix to look one specific way, now finalizes an already-active touch capture before restarting when new contacts take over the same update, prefers passing only the actually released tracked contacts into the touch-up handoff when release and next-press share one raw update, suppresses repeated touch-capture restarts during the same very-short handoff burst without delaying genuine finger-count increases, and treats composite/tap follow-up waits as valid only when the matched gesture action is actually executable for the current device/condition; high-frequency touchscreen validation is still required. |
| [#94](https://github.com/TransposonY/GestureSign/issues/94) | Simple pen gestures can now use `Tip` or `Hover` without requiring the barrel/right-click button or inverted pen as a mandatory activation gate, and hover-only pen strokes no longer drop their active state when the tip later touches the screen. |
| [#127](https://github.com/TransposonY/GestureSign/issues/127), [#119](https://github.com/TransposonY/GestureSign/issues/119), [#112](https://github.com/TransposonY/GestureSign/issues/112), [#55](https://github.com/TransposonY/GestureSign/issues/55) | Pen settings and docs are clearer, simple pen gestures no longer require a barrel button, and standard `PenUsage` devices are no longer rejected only because their raw HID path contains `ROOT` or `VIRTUAL_DIGITIZER`; Wacom/passive pen support still depends on whether the driver exposes compatible HID pen or touchpad input. |
| [#135](https://github.com/TransposonY/GestureSign/issues/135), [#116](https://github.com/TransposonY/GestureSign/issues/116), [#114](https://github.com/TransposonY/GestureSign/issues/114), [#59](https://github.com/TransposonY/GestureSign/issues/59) | Precision touchpad handling is improved, including safer one-finger capture gating that checks app-specific conditioned touchpad actions before allowed global fallback, standard `TouchPadUsage` devices are no longer rejected only because their raw HID device path contains `ROOT` or `VIRTUAL_DIGITIZER`, obvious trackpad-like digitizer reports can now be routed through the touchpad path even when their raw top-level usage looks more like `TouchScreenUsage` or another nonstandard digitizer collection, touch parsing now walks only link collections that actually expose coordinate caps instead of blindly treating every child as a contact, same-class touchpad sources are tracked by device handle so multiple touchpads are less likely to interfere with each other, and raw input inventory plus digitizer-validation logs now capture vendor/product plus raw/effective usage details to speed up Win11 third-party-driver diagnosis; vendor-specific support still needs per-device validation. |
| [#130](https://github.com/TransposonY/GestureSign/issues/130), [#87](https://github.com/TransposonY/GestureSign/issues/87), [#45](https://github.com/TransposonY/GestureSign/issues/45) | Mouse hold-down actions triggered during continuous capture are now auto-released on capture end/cancel to reduce stuck-button risk, and the hold-down tracking now follows the capture state that existed when the continuous action fired instead of relying on the later async command-execution moment; full OS-level drag-window/file-drag workflows still need hardware/UI validation. |
...
- Every push to `master` also updates the `continuous` prerelease with the latest installer and portable builds for manual testing.
...
- You can also run the `Release` workflow manually from GitHub Actions. Provide `tag_name`; use `continuous` for a prerelease test build, or a semver-like tag for a formal release. You can optionally provide `build_ref` to build a specific branch, commit, or tag. If `build_ref` is omitted, semver-like tags default to building that tag, while `continuous` defaults to the SHA of the ref you launched the workflow from.
