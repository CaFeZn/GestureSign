...
| [#109](https://github.com/TransposonY/GestureSign/issues/109), [#131](https://github.com/TransposonY/GestureSign/issues/131) | Shell/taskbar activation and modifier-key cleanup around switch-window and switch-desktop actions have been hardened. Built-in Alt+Tab / Shift+Alt+Tab / Win+Ctrl+Arrow actions, and equivalent `Hot Key` shortcuts, now force-reset the relevant modifiers on the success path as well instead of only relying on best-effort key-up injection, that cleanup no longer tries to restore the pre-switch target window after the system window/desktop switch command has already changed focus, and generic command execution now re-resolves shell/taskbar foreground drift back to the real captured target before sending keystrokes. |
