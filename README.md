...
- Cursor-driven pen and touchpad fallback paths now prefer the cursor's current monitor before the foreground-window monitor, so their coordinates stay aligned with the same display used by cursor-based app matching and touchpad start conditions.
