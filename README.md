...
| [#85](https://github.com/TransposonY/GestureSign/issues/85) | Touchpad delay, duplicate, and jagged-path reports are still partly driver/device-specific, but touchpad/raw-input handling has improved: full logical-slot parsing, per-device-handle tracking, and stable per-contact ordering for live multi-finger updates reduce false back-and-forth motion or repeated continuous-trigger direction flips on some hardware. Affected Synaptics or vendor-driver hardware is still required for full closure. |
