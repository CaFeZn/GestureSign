...
| [#75](https://github.com/TransposonY/GestureSign/issues/75), [#32](https://github.com/TransposonY/GestureSign/issues/32) | Trigger conditions support touch-coordinate variables, multi-condition expressions, and percent coordinates aligned with Win32 virtual-screen pixel bounds. Multi-contact condition evaluation now also reads points and contact identifiers from one ordered capture snapshot, so variables such as `finger_2_start_X` are less likely to drift onto the wrong tracked contact during later execution paths. |
