...
| [#109](https://github.com/TransposonY/GestureSign/issues/109), [#131](https://github.com/TransposonY/GestureSign/issues/131) | Shell/taskbar activation and modifier-key cleanup around switch-window and switch-desktop actions have been hardened. Built-in Alt+Tab / Shift+Alt+Tab / Win+Ctrl+Arrow actions, and equivalent `Hot Key` shortcuts, now force-reset the relevant modifiers on the success path as well instead of only relying on best-effort key-up injection, that cleanup now preserves the real foreground window after the system switch has already completed instead of leaving focus on a temporary shield window or restoring the pre-switch target, and generic command execution now re-resolves shell/taskbar foreground drift back to the real captured target before sending keystrokes. |
