...
- If browser actions do not match Chromium Edge, add or edit an application rule using executable filename or process matching for `msedge.exe`. The bundled browser group matches common browser processes including `msedge`, `chrome`, `firefox`, `iexplore`, and legacy `MicrosoftEdge`. Detached Firefox tab windows should still match by executable, but failures need per-window validation because browser window classes and tab-tearoff behavior vary by version.
...
| [#103](https://github.com/TransposonY/GestureSign/issues/103) | Display-change handling resets touchscreen screen caches and the gesture-trail surface, but external-monitor hot-unplug crash reports still need validation on the reported laptop/external-display topology. |
