...
namespace GestureSign.ControlPanel.Dialogs
{
    ...
    public partial class ActionDialog : MetroWindow
    {
        ...
        private bool SaveAction()
        {
            try
            {
                var regex = new Regex("finger_[0-9]+_((start|end)_[XY]%?|ID)|window_is_(maximized|minimized|fullscreen)|key_is_(shift|ctrl|alt|win)_down|key_[A-Za-z0-9]+(_[A-Za-z0-9]+)*_down");
                var replaced = regex.Replace(ConditionTextBox.Text, m => m.Value.StartsWith("finger_") ? "10" : "(1=0)");
